...
- プロジェクトでローカル Supabase を起動:
  ...
  supabase start
  ```
  表示された **API URL**（例: `http://127.0.0.1:54321`）と **anon key** を `.env.local` に設定する。マイグレーションは `supabase start` 時に自動適用される。必要なら `supabase db reset` で再適用できる。
...
- 未ログイン時は案件一覧は空で、ログインを促すメッセージを表示
